                    ...
                    for i5 in rrange(6):
                        for i6 in rrange(7):
                            for i7 in rrange(8):
                                for i8 in rrange(9):
                                    '''
                                    for i9 in rrange(36 - i):
                                        dd = d + [9] * i9
                                        PREFIXES[i + i9].append(d)
                                    '''
                                    i = i1 + i2 + i3 + i4 + i5 + i6 + i7 + i8
                                    if not i:
                                        continue
                                    d = [1] * i1 + [2] * i2 + [3] * i3 + [4] * i4 + [5] * i5 + [6] * i6 + [
                                        7] * i7 + [8] * i8
                                    digi.num = d
                                    n = reduce(lambda x, y: x * 10 + y, d)
                                    f_ = f(digi.num)
                                    fsum = digits_sum(f_)
                                    single = single_digit_sum(f_)
                                    pp = P[single]
                                    if pp.get(fsum):
                                        pp.get(fsum).append((str(n),f_))
                                    else:
                                        pp[fsum] = [(str(n), f_)]

    for i in range(1, 10):
        for k, v in P[i].items():
            P[i][k].sort(key=lambda x: int(x[0]))
            print(i, k, len(v))
            print(v[:min(5, len(v))])
...
class Digits:
    ...
    def next_new(self):
        if self.num[self.suffix_len] == 8:
            self.numself[self.suffix_len] = 9
            return self
        self.prefix_pos += 1
        if self.prefix_pos < self.prefix_size:
            prefix = PREFIXES[self.prefix_len][self.prefix_pos]
            for i in range(self.prefix_len):
                self.num[i + self.suffix_len] = prefix[i]
            return self
        elif self.prefix_len and False:
    ...
    def next_prefixes(self, start_digit=0):
        self.prefix_pos += 1
        if self.prefix_pos < self.prfix_size:
            for i in range(self.prefix_len):
                self.num[self.suffix_len+i] = PREFIXES[self.prefix_pos][-i-1]
        elif self.prefix_len < 36:
            self.num.append(0)
            self.prefix_len += 1
            self.prefix_pos = 0
            self.prefix_size = len[PREFIXES[self.prefix_len]]
